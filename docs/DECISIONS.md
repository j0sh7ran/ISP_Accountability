...
## 2026-09-20 — Linux/macOS support

| Decision | Choice | Rationale / notes |
|---|---|---|
| Route tracing | `apps/measurements/probes/route.py`'s `run_traceroute()` now dispatches on `sys.platform`: Windows keeps `tracert -d`; Linux/macOS shell out to `traceroute -n -m <hops> -w <seconds>` via a new `parse_traceroute_output()` parser | Unix `traceroute` output puts the IP address right after the hop number (not last, as `tracert` does) and prints a fully-timed-out hop as `* * *` rather than "Request timed out" — different enough to need its own parser rather than a shared one. `traceroute` isn't always preinstalled on Linux (documented in SETUP_AND_OPERATIONS.md), unlike Windows' bundled `tracert.exe` and macOS's bundled `traceroute`. |
| MTU/PMTUD | `apps/measurements/probes/mtu.py` split into `_ping_df_command()`/`_ping_success()`/`_is_fragmentation_needed()`, each branching on `sys.platform`: Windows `ping -f -l <size> -w <ms>` (success = `"Reply from"` + `"TTL="`); Linux (iputils) `ping -M do -s <size> -W <seconds>` (success = `"bytes from"`); macOS (BSD) `ping -D -s <size> -W <ms>` (success = `"bytes from"`) | Each OS's `ping` has different flags for the DF bit, payload size, and timeout units (ms vs whole seconds), and different success/failure text — but the *payload-size semantics* and the 1200–1500-byte binary-search range are identical across all three, so only command-building/parsing needed to change, not `run_mtu_probe()`'s search algorithm. |
| Elevation detection | `apps/core/elevation.py`'s `is_elevated()` now also checks `os.geteuid() == 0` when `sys.platform in ('linux', 'darwin')`, still returning `None` for anything unrecognized | Mirrors the existing Windows `IsUserAnAdmin()` check with the POSIX equivalent (root check) rather than leaving Linux/macOS permanently reporting "unknown". |
